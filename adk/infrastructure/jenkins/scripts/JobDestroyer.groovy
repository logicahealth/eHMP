...
class JobDestroyer {
    ...
    def run(){
        ...
        jenkinsMain.job.grep{ job -> job.name.text().contains('adk') && job.name.text() != 'creator' && job.name.text() != 'destroyer' && !job.name.text().contains('template') && !job.name.text().contains('infrastructure')}.each{ job ->
