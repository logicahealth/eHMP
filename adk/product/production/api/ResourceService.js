define([
    ...
    Backbone.PageableCollection.prototype.parse = function (response) {
        ...
        if(_.isFunction(collectionConfigCollectionParse)) {
            collectionConfigCollectionParse = _.bind(collectionConfigCollectionParse, this.fetchOptions.collectionConfig);
        }
        var collectionParse = this.collectionParse || collectionConfigCollectionParse;
        if (response.data) {
            if (response.data.items) {
                parsedResponse = response.data.items;
            } else {
