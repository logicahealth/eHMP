...
public class CDSInvoker implements CDSInvocationIFace {
			...
			if (intentMapping == null && repositoryLookupAgent != null) {
				try {
				   intentMapping = repositoryLookupAgent.lookupIntent(intent);
					...
					this.addConfigurationError(resultBundle, "Mongodb connection error,", "Intent look up failed");
					return resultBundle;
				}
			}
				...
				if (invocationMappingList == null) {
					...
					for (Iterator<InvocationMapping> iterator2 = invocationMappingList.iterator(); iterator2.hasNext();) {
						...
						if (engineInfo == null && repositoryLookupAgent != null) {
						   try {
							   engineInfo = repositoryLookupAgent.lookupEngine(engine);
								...
								this.addConfigurationError(resultBundle, "Mongodb connection error,", "Engine look up for " + engine + " failed");
								return resultBundle;
							}
						}
						...
						if (engineInfo == null) {
							...
							addConfigurationError(resultBundle, engine + " is not found " , engine + "is not a configured engine name for intent " + intent);
							break;
						}
	...
	private void addConfigurationError(ResultBundle resultBundle, String reason, String message) {
		FaultInfo fault = new FaultInfo();
		String errorMessage = InvocationConstants.StatusCodes.CONFIGURATION_ERROR + ", Reason = " + reason + "  " + message;
		fault.setFault(errorMessage);
		resultBundle.getFaultInfo().add(fault);
		updateBundleStatus(resultBundle, StatusCode.CONFIGURATION_ERROR);
		LOGGER.error(errorMessage);
