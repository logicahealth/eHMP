...
When(/^the "(.*?)" is requested with parameter "(.*?)" value "(.*?)" for patient "(.*?)"$/) do |observation, date, value, patient|
  ...
  type = { "Content-Type" => "application/json" }
  @response = HTTPartyWithBasicAuth.get_json_with_authorization(encoded, type)
end
...
entryname = ""
When(/^user sends POST request to create cds engine registory entry with payload "(.*?)"$/) do |payload|
  temp = QueryRDKCDS.new
