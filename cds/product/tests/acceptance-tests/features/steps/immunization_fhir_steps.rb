...
When(/^the client requests observation for the patient "(.*?)" in FHIR format$/) do |pid|
  temp = QueryRDKCDSfhir.new
...
When(/^the observation is requested with parameter code value "(.*?)" for patient "(.*?)"$/) do |code, pid|
  temp = QueryRDKCDSfhir.new
  path = temp.path + "/patient/#{pid}/observation?code=#{code}"
  payload = ""
  type = { "Content-Type" => "application/json" }
  @response = HTTPartyWithBasicAuth.get_json_with_authorization(path, payload, type)
  # puts @response.body
