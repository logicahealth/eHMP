...
class JobManager {
    ...
    def run(){
        ...
        dir.traverse(
            ...
            } else if (System.getenv('JENKINS_URL').contains('va.gov') ^ kinBuildFilePath.name.contains('ede')) {
                println "skipping this kin file: if we are in ede, the kin file is not for ede; if we are not in ede, the kin file is for ede"
            } else {
