...
action :execute do
  ...
  if !node[:jds][:jds_data][:data_bag][:trustsys].nil?
    list = data_bag_item('jds_data',node[:jds][:jds_data][:data_bag][:trustsys]).to_hash["override"]
  else
  ...
  list.each{ |item|
    ruby_block "check if #{item['uid']} exists" do
      ...
      only_if { item_exists?("http://localhost:#{new_resource.port}/#{store}/#{item['uid']}", "name") }
    end
    ...
    http_request "#{item["uid"]}_put" do
      ...
      action :put
      not_if { item_exists?("http://localhost:#{new_resource.port}/#{store}/#{item['uid']}", "name") }
    end
  }
