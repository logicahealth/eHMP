...
############################################## Staging Artifacts #############################################
if ENV.has_key?('CDSI_METRICS_VERSION_FILE')
  raise "Dev deploys are not supported."
else
  cdsi_metrics_source = artifact_url(node[:cds_provision][:artifacts][:cdsi_metrics])
end

if ENV.has_key?('CDSI_RESULTS_VERSION_FILE')
  raise "Dev deploys are not supported."
else
  cdsi_results_source = artifact_url(node[:cds_provision][:artifacts][:cdsi_results])
end

if ENV.has_key?('OPENCDS_KNOWLEDGE_REPOSITORY_DATA_FILE')
  raise "Dev deploys are not supported."
else
