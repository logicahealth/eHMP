...
version = ENV['cdsinvocation_VERSION']

machine_ident = "cdsdb"

boot_options = node[:cds_provision][machine_ident.to_sym]["#{node[:machine][:driver]}".to_sym]
node.default[:cds_provision][:cdsdb][:copy_files].merge!(node[:machine][:copy_files])
...
if node[:machine][:driver] == "ssh" && node[:simulated_ssh_driver].nil?
  dev_cdsdb = false
else
  dev_cdsdb = true
end
