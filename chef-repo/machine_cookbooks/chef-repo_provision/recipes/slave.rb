...
node.default[:machine][:convergence_options].merge! ({
  install_sh_url: "#{node[:common][:nexus_url]}/nexus/content/repositories/environment/vistacore/chef-install/install/1.0.5.slave/install-1.0.5.slave.sh"
})
...
machine_options = {
  :bootstrap_options => {
    ...
    :subnet_id => "DNS",
    :security_group_ids => ["GROUP ID", "GROUP ID"]
  },
  :image_id => "ami-826b0b95",
  :ssh_username => "USER    ",
...
machine "#{node[:machine][:stack]}" do
  ...
  attributes(
    ...
    workstation: {
      ...
      slave: {
        executors: ENV["SLAVE_EXECUTORS"] || "4"
      }
    }
  )
