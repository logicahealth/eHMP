...
####################################################### Shared Folders #########################################################
if ENV['DEV_DEPLOY']
  node.default[:rdk_provision][:activity_handler][:vagrant][:shared_folders].push(
    {
...
machine_ident = ENV['ACTIVITY_HANDLER_IDENT'] || "activity-handler"

boot_options = node[:rdk_provision][:activity_handler]["#{node[:machine][:driver]}".to_sym]
node.default[:rdk_provision][:activity_handler][:copy_files].merge!(node[:machine][:copy_files])
...
r_list << "role[activity_handler]"
r_list << "role[vista_aso_rejector]"
r_list << "recipe[postfix_wrapper]" if ENV.has_key?("CONFIGURE_POSTFIX")
r_list << "recipe[activity_handler::clear_logs@#{rdk_deps["activity_handler"]}]" if node[:machine][:driver] == "aws"
r_list << "recipe[activity_handler@#{rdk_deps["activity_handler"]}]"
...
machine machine_name do
  ...
  attributes(
    ...
    db_env: {
      :activity_handler_env => ENV['ACTIVITY_HANDLER_DB_ITEM']
    },
    activity_handler: {
  ...
  files lazy { node[:rdk_provision][:activity_handler][:copy_files] }
