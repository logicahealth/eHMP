...
####################################################### Shared Folders #########################################################
if ENV['DEV_DEPLOY']
  node.default[:rdk_provision][:fetch_server][:vagrant][:shared_folders].push(
    {
...
machine_ident = ENV['FETCH_SERVER_IDENT'] || "fetch-server"

boot_options = node[:rdk_provision][:fetch_server]["#{node[:machine][:driver]}".to_sym]
node.default[:rdk_provision][:fetch_server][:copy_files].merge!(node[:machine][:copy_files])
...
r_list << "role[fetch_server]"
r_list << "role[vista_aso_rejector]"
r_list << "recipe[postfix_wrapper]" if ENV.has_key?("CONFIGURE_POSTFIX")
r_list << "recipe[fetch_server::clear_logs@#{rdk_deps["fetch_server"]}]" if node[:machine][:driver] == "aws"
r_list << "recipe[fetch_server@#{rdk_deps["fetch_server"]}]"
...
machine machine_name do
  ...
  attributes(
    ...
    db_env: {
      :fetch_server_env => ENV['FETCH_SERVER_DB_ITEM']
    },
    fetch_server: {
  ...
  files lazy { node[:rdk_provision][:fetch_server][:copy_files] }
