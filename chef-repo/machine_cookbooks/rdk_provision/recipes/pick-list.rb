...
####################################################### Shared Folders #########################################################
if ENV['DEV_DEPLOY']
  node.default[:rdk_provision][:pick_list][:vagrant][:shared_folders].push(
    {
...
machine_ident = ENV['PICK_LIST_IDENT'] || "pick-list"

boot_options = node[:rdk_provision][:pick_list]["#{node[:machine][:driver]}".to_sym]
node.default[:rdk_provision][:pick_list][:copy_files].merge!(node[:machine][:copy_files])
...
r_list << "role[pick_list]"
r_list << "role[vista_aso_rejector]"
r_list << "recipe[postfix_wrapper]" if ENV.has_key?("CONFIGURE_POSTFIX")
r_list << "recipe[pick_list::clear_logs@#{rdk_deps["pick_list"]}]" if node[:machine][:driver] == "aws"
r_list << "recipe[pick_list@#{rdk_deps["pick_list"]}]"
...
machine machine_name do
  ...
  attributes(
    ...
    db_env: {
      :pick_list_env => ENV['PICK_LIST_DB_ITEM']
    },
    pick_list: {
  ...
  files lazy { node[:rdk_provision][:pick_list][:copy_files] }
