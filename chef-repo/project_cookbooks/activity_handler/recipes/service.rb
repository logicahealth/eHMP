...
template "/etc/bluepill/#{config[:name]}.pill" do
  ...
  variables(
    :name => config[:name],
    :working_directory => node[:activity_handler][:home_dir],
    :deploy_path => config[:deploy_path],
    ...
    :log_directory => node[:activity_handler][:log_dir],
    :pid_directory => node[:activity_handler][:pid_dir],
    :tls_reject_unauthorized => node[:activity_handler][:tls_reject_unauthorized],
    :uv_threadpool_size => node[:activity_handler][:uv_threadpool_size],
    :apm_deploy => node[:activity_handler][:enable_apm],
    :app_dynamics_deploy => node[:activity_handler][:enable_app_dynamics],
