...
action :execute do
	...
	template pcmm_file do
	  variables(
	    ...
	    :mssql_username => node['jbpm']['sql_users']['mssql_username'],
	    :refresh_view_minutes => node['jbpm']['oracle_config']['refresh_view_minutes']
	  )
	  sensitive true
	end
	...
	execute "Create_pcmm_from_sql_file" do
		cwd "/tmp"
		command "sudo -Eu oracle PATH=$PATH echo exit | sqlplus sys/#{datasource_password} as sysdba@connect @#{pcmm_file}"
		sensitive true
	end
