...
module.exports =
  ...
  'matches IPv4 CIDR correctly': (test) ->
    ...
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 8),  true)
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 8),  true)
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 8), true)
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 16), true)
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 16), false)
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 15), true)
    test.equal(addr.match(ipaddr.IPv4.parse('IP      '), 32), false)
    test.equal(addr.match(addr, 32), true)
    test.done()

  'detects reserved IPv4 networks': (test) ->
    test.equal(ipaddr.IPv4.parse('IP      ').range(),        'private')
