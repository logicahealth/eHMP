...
module.exports =
  ...
  'matches IPv4 CIDR correctly': (test) ->
    ...
    test.equal(addr.match(ipaddr.IPv4.parse('IPADDRES'), 8),  true)
    test.equal(addr.match(ipaddr.IPv4.parse('IPADDRESS'), 8), true)
    ...
    test.equal(addr.match(ipaddr.IPv4.parse('IPADDRES'), 32), false)
    test.equal(addr.match(addr, 32), true)
    test.done()

  'detects reserved IPv4 networks': (test) ->
    test.equal(ipaddr.IPv4.parse('IPADDRES').range(),        'private')
