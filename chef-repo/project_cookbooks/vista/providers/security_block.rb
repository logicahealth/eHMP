    ...
    block do
      begin
        ...
        shell.wait_for(:output, /Import from file name/) do | process, match |
          process.write("#{node[:vista][:cache_mgr_dir]}/EncryptAudit.xml\n")
          Chef::Log.info("File to import:  \"#{node[:vista][:cache_mgr_dir]}/EncryptAudit.xml\"")
        end
        ...
        shell.wait_for(:output, /Confirm import of security records from #{node[:vista][:cache_mgr_dir]}\/EncryptAudit.xml/) do | process, match |
          process.write("Yes\n")
        ...
        shell.wait_for(:output, /Import from file name/) do | process, match |
          process.write("#{node[:vista][:cache_mgr_dir]}/SecureConfiguration.xml\n")
          Chef::Log.info("File to import:  \"#{node[:vista][:cache_mgr_dir]}/SecureConfiguration.xml\"")
        end
        shell.wait_for(:output, /Parameters\?/) do | process, match |
          process.write("\n")
          Chef::Log.info("Accepted default parameter")
        ...
        shell.wait_for(:output, /Confirm import of security records from #{node[:vista][:cache_mgr_dir]}\/SecureConfiguration.xml/) do | process, match |
          process.write("Yes\n")
          Chef::Log.info("Confirm import")
        end
        shell.wait_for(:output, /Option\?/) do | process, match |
