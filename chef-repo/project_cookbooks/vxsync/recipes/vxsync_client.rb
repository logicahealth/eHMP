...
cron "record_retirement" do
  minute node[:vxsync][:client][:record_retirement][:minute]
  hour node[:vxsync][:client][:record_retirement][:hour]
  weekday node[:vxsync][:client][:record_retirement][:weekday]
  command "cd #{node[:vxsync][:client][:home_dir]}; /usr/local/bin/node ./utils/patient-record-retirement/run-patient-record-retirement-util.js >> #{node[:vxsync][:client][:log_directory]}/#{node[:vxsync][:client][:record_retirement][:log_file]}"
  action node[:vxsync][:client][:record_retirement][:enabled] ? :create : :delete
end
