...
class JobDestroyer {
  ...
  def remoteBranches = []
    ...
    jenkinsMain.job.grep{ job -> job.name.text().contains('communications-cli') && job.name.text() != 'creator' && job.name.text() != 'destroyer' && !job.name.text().contains('template') && !job.name.text().contains('infrastructure')}.each{ job ->
