...
class JobDestroyer {
    ...
    def run(){
        ...
        jenkinsMain.job.grep{ job -> job.name.text().contains('ehmp-ui') && job.name.text() != 'creator' && job.name.text() != 'destroyer' && !job.name.text().contains('template') && !job.name.text().contains('infrastructure')}.each{ job ->
