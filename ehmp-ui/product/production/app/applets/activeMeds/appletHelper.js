	...
	var appletHelper = {
		...
		getFillableStatus: function(medDomainData) {
			...
			if (vaStatus === "expired") {
				...
				return "0 Refills";
			} else if (!medDomainData.lastFilled && !medDomainData.orders[0].fillsRemaining && _.isEmpty(medDomainData.fills)) {
				return "No Data";
			}
		...
		getNextAdminData: function(medDomainData) {
            ...
            if (medDomainData.nextAdminStatus === "Expired") {
                var expired = this.getTimeSince(medDomainData.stopped, true);
                ...
                var discontinued = this.getTimeSince(medDomainData.stopped, true);
		...
		getTimeSince: function(dateString, showMinutes) {
		...
		getEarlierStopAsMoment: function(medDomainData) {
			var overallStop = moment(medDomainData.overallStop, 'YYYYMMDDHHmm');
			var stopped = moment(medDomainData.stopped, 'YYYYMMDDHHmm');
			var overallStart = moment(medDomainData.overallStart, 'YYYYMMDDHHmm');
		...
		getFillableData: function(medDomainData) {
			var stopped = this.getEarlierStopAsMoment(medDomainData);
			...
			var fillsRemaining = medDomainData.orders[0].fillsRemaining;
			...
			if (status !== "Fillable for") {
				...
				if (status === "Expired") {
					var expired = this.getTimeSince(stopped, true);
					result.hasLabel = true;
					result.label = 'label label-danger';
					if (expired.count < 0) {
						result.date = '??';
					...
					var discontinued = this.getTimeSince(stopped, true);
					...
					result.display = medDomainData.fillableStatus = 'No Data';
					result.label = 'label label-danger';
					result.hasLabel = true;
				}
				return result;
			...
			if (fillableTimeMinutes < 1440) {
				fillableTimeString = '1 d';
			} else if (fillableTimeMinutes >= 1440 && fillableTimeMinutes <= 86400) {
