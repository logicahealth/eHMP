...
define([
    ...
    var appletHelper = {
        ...
        getGraphTextInfo: function(models) {
            ...
            var numberOfFacilities = models.length;
            var medOrderInfo = '';
            ...
            medOrderInfo = 'with overlapping orders from ' + numberOfFacilities + ' facilities. ' + medOrderInfo;
        ...
        isPRN: function(medDomainData) {
            if (medDomainData.sig && medDomainData.sig.toString().toUpperCase().indexOf('AS NEEDED') !== -1) {
                ...
                return !!(medDomainData.scheduleName && medDomainData.scheduleName.toString().indexOf('PRN') !== -1);
            }
        },
        hasFillDetail: function(medDomainData) {
            if (medDomainData.vaType === 'O' || medDomainData.supply) {
                return !!_.get(medDomainData, 'orders[0].fillsAllowed');
            } else {
                return false;
            }
        },
        hasScheduledTimes: function(medDomainData) {
            return medDomainData.vaType === 'I';

        },
        hasDailyDoseInfo: function(medDomainData) {
            if (medDomainData.scheduleFreq) {
                if (medDomainData.dose) {
                    return !!(medDomainData.vaType === 'O' || medDomainData.vaType === 'N');
                } else {
                    return false;
                }
            }
        ...
        getDailyOrScheduledDose: function(medDomainData) {
            if (this.hasScheduledTimes(medDomainData)) {
                ...
                var MIN_PER_DAY = 1440; // 24 * 60
                return MIN_PER_DAY / medDomainData.scheduleFreq * parseInt(medDomainData.dose) + ' ' + medDomainData.units;
            }
            return false;
        },
        ...
        getStandardizedFacilityCode: function(medDomainData) {
        ...
        getNextAdminData: function(medDomainData) {
            ...
            if (medDomainData.nextAdminStatus === 'Expired') {
                ...
                result.hasLabel = true;
            } else if (medDomainData.nextAdminStatus === 'Pending') {
        ...
        getLastFill: function(medDomainData) {
            if (_.isUndefined(medDomainData.lastFilled)) {
                if (medDomainData.fills.length > 0) {
                    if (medDomainData.fills.length > 1) {
                        _.sortBy(medDomainData.fills, function(d) {
        ...
        hasIngrediantCodeName: function(medDomainData) {
            return  _.get(medDomainData, ['products', 0, 'ingredientCodeName']);
        },
        ...
        parseMedResponse: function(response) {
            ...
            if (response.orders !== undefined && response.orders[0] !== undefined) {
                ...
                if (response.orders[0].orderUid !== undefined) {
                    response.orderUid = appletHelper.sliceString(' \'' + response.orders[0].orderUid + ' \'');
                }
            ...
            response.standardizedFacilityCode = appletHelper.getStandardizedFacilityCode(response);
