    ...
    function chartBuilder(model, isStackedGraph) {
        ...
        for (var t = 0; t < facilityCollection.length; t++) {
            ...
            for (var u = 0; u < facilityModelCollection.length; u++) {
                ...
                var overallStart, stopped, releaseDate, vaStatus, calculatedStopDate, vaType;
                ...
                if (overallStart > 0) {
                    var startMoment = moment(overallStart);
                    if (startMoment.isSame(stopped)) {
                        stopped = moment(stopped).add(1, 'minutes').valueOf();
                    }
                    if (startMoment.isBefore(stopped) || startMoment.isSame(stopped)) {
                        ...
                        if (vaStatus === 'pending') {
                            ...
                            pushToLocalGraph(twenty, checkBeforeOldestDates(overallStart, stopped, oldest), checkAfterNewestDates(overallStart, stopped, newest), white, t, 0, facilityMoniker, white);
                            ...
                            if (moment(stopped).isBefore(calculatedStopDate)) {
                                ...
                                pushToLocalGraph(twenty, checkBeforeOldestDates(stopped, calculatedStopDate, oldest), checkAfterNewestDates(stopped, calculatedStopDate, newest), color, t, 0, facilityMoniker, color);
                            }
                            if (vaType === 'O') {
                                if (fills) {
        ...
        if (isStackedGraph && localListGraphData.chart.height < 100) {
