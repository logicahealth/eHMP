define([
    ...
    var writebackUtils = {
        buildSaveProblemsModel: function(model, userSession, currentPatient, saveProblemModel) {
            ...
            saveProblemModel.set('pid', currentPatient.get('pid'));
        ...
        buildTreatmentFactorsObject: function(model) {
            ...
            if (model.get('noTreatmentFactors') !== 'true') {
                ...
                var headOrNeckCancer = model.get('treatmentFactors').findWhere({
                ...
                var ionizingRadiation = model.get('treatmentFactors').findWhere({
                ...
                var agentOrange = model.get('treatmentFactors').findWhere({
                    name: 'agent-orange'
                });
                if (agentOrange) {
                ...
                var swAsia = model.get('treatmentFactors').findWhere({
                    name: 'sw-asia'
                });
                if (swAsia) {
                ...
                var mst = model.get('treatmentFactors').findWhere({
                    name: 'mst'
                });
                if (mst) {
                ...
                var shipboard = model.get('treatmentFactors').findWhere({
                ...
                var serviceConnected = model.get('treatmentFactors').findWhere({
        ...
        buildIncomingComments: function(originalCommentCollection, newCommentCollection, userSession) {
            ...
            if (!_.isUndefined(originalCommentCollection) && originalCommentCollection.length > 0) {
                originalCommentCollection.each(function(originalComment) {
                    ...
                    var newComment = newCommentCollection.findWhere({
