define([
    ...
    describe('Permission Set UMA Collection', function() {
        var collection;
        var appendValue = function() {
            var original = mock.setCollection(true);
            return _.map(original, function(item) {
                item.value = item.val;
                return item;
            });
        };
        beforeEach(function() {
            spy.StringFormatter();
            collection = new Collection(mock.setCollection(true), {
                parse: true
            });
        });

        it('can look up a model by uid', function() {
            var original = appendValue();
        ...
        it('can find containing permissions', function() {
            ...
            expect(withCommonPermission.length).toBe(3);
        ...
        it('creates an array of unique permissions correctly', function() {
            var data = appendValue();
            var uids = [data[0].uid, data[1].uid];
            var permissions = collection.getPermissions(uids);
            expect(permissions.length).toEqual(11);
        });

        it('creates an array of unique permissions correctly from labels', function() {
            var data = appendValue();
            var labels = [data[0].label, data[1].label];
            var permissions = collection.getPermissions(labels, 'label');
            expect(permissions.length).toEqual(11);
        });
        
        it('creates an array of unique permissions from picklist selected values correctly', function() {
            var data = appendValue();
            var preselect = [data[0].uid, data[1].uid];
            var picklist = collection.toPicklist(preselect);
            var permissions = picklist.getSelectedPermissions();
            expect(permissions.length).toEqual(11);
        });
    });
});