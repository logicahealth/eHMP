...
@f144_numeric_lab_results_modal_graph_reference_range_display @US2504 @TA7969 @manual @UI @modal_test
Scenario: Numeric Lab Results Modal - ensure data ranges are appropriate for number of tests.
  #There is a defect to track the numeric lab results modal graph issue.
...
@f144_numeric_lab_results_modal_graph_clear_date_axis_labels @US2982 @TA9721 @manual @UI @modal_test
Scenario: Numeric Lab Results Modal - ensure dates are clearly displayed even when zoomed.
  #There is a defect to track the numeric lab results modal graph issue.  also we need to identify a patient with Panel data.
  Given user searches for and selects "Seven,Patient"
  Then Cover Sheet is active
  Given the user has selected All within the global date picker
  And the user clicks the control "Expand View" in the "Numeric Lab Results applet"
  #And the user clicks the first Panel result in the Numeric Lab Results applet
  ...
  #When the user zooms in the "Numeric Lab Results applet" graph manually
