@f662_pob_access_control_write_back  @DE4326 @DE4560 @reg3 @update_perm
...
Scenario: Validate admin applet form fields.
  ...
  Then POB Access Control applet modal title says "USERS FOR PANORAMA (SITE)"
...
Scenario: Authorized users can view the Notes Applet
  ...
  And permission set "Standard Doctor" is selected
  And the authorized user saves the permissions
  Then the Select Permissions Window closes
  And the Permission Sets sections includes set "Standard Doctor"
  Then POB log me out
...
Scenario: Authorized users cannot edit their own permission sets
  ...
  And POB authorized user cannot edit the permission sets


@f662_pob_user_remove_all_permission_sets @add_remove 
Scenario: Authorized users can make a vista user inactive
  ...
  And the authorized user saves the permissions
  Then the Select Permissions Window closes
  Then POB user "TRACY KEELEY" has following permission sets
    | permission sets           |
    | None            |
...
@f662_pob_user_add_back_permission_sets 
Scenario: Authorized users can add user permission sets
  Given POB user is logged into EHMP-UI with facility as  "PANORAMA" accesscode as  "USER  " verifycode as  "PW      "
  And staff view screen is displayed
  And POB user views the Access Control Applet
  And POB user expands Access Control Applet
  ...
  And POB authorized user edits the permission sets
  And permission set "Read Access" is selected
  And permission set "Pharmacist" is selected
  And the authorized user saves the permissions
  Then the Select Permissions Window closes
