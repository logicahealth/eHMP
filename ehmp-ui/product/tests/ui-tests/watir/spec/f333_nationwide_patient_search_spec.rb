...
describe 'feature#f333|defects#DE1205,DE1203: f333_nationwide_patient_search_spec.rb (DE1491, DE1722)', future: true do
  ...
  context 'TC#TC561: Verify that the Demographic information for secondary patients displays in demographic detail drop-down' do
    ...
    patient_search_scenario.each do |scenario|
      it '. Type in patient name, click the search, select the patient from the table. Then verify that the patient full name displays in the Confirmation Section header' do
        ...
        Watir::Wait.until { search.this_patient_in_the_nationwide_patient_list_table?(1, patient_name) }
        search.click_the_right_patient_from_nationwide_table(patient_name)
      ...
      it '. Keep clicking the patient and click Confirm Selection, until patient data is loaded from JDS, then verify user lands on the overview screen' do
        ...
        while search.errorMsgAfterFirstConfirm? && (search.errorMsgAfterFirstConfirm.include?('record is not yet accessible'))
          search.click_the_right_patient_from_nationwide_table("#{scenario[:lName].strip}, #{scenario[:fName]}")
          search.firstConfirm_element.when_visible(SMALL_TIMEOUT)
          search.firstConfirm
          sleep 2
          i += 1
      ...
      it part1 + part2 do
        overview.patientHomePhone_element.when_visible(LARGE_TIMEOUT)
  ...
  xcontext 'TC#TC560: Verify that the Write Back is disabled from CoverSheet for Non-Vista patient.' do
  ...
  xcontext 'TC#TC560: Verify that the Write Back is disabled from the expanded applet view for Non-Vista patient.' do
