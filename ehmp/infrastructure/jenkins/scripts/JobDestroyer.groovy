...
class JobDestroyer {
    ...
    def run(){
        ...
        jenkinsMain.job.grep{ job -> !job.name.text().startsWith('ehmp-ui') && job.name.text().startsWith('ehmp') && job.name.text() != 'ehmp-creator' && job.name.text() != 'ehmp-destroyer' && job.name.text() != 'ehmp-workspace-management' && !job.name.text().endsWith('template') && !job.name.text().contains('infrastructure')}.each{ job ->
