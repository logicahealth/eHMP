...
var fs = require('fs');
var medsMapper = require('./lib/mapsv3/map10160-0.js');
var demoMapper = require('./lib/mapsv3/map52536-0.js');
...
var domainNames = Object.keys(utils.domainsv4);
for (var domainTypeNum = 0; domainTypeNum < domainNames.length; domainTypeNum++) {
    // get the list of files
    var fileList = utils.getJSONFileListingByLOINC(utils.domainsv4[domainNames[domainTypeNum]].v2loinc, "../src/main/resources/data/");

    // loop through the list of files and generate the maps
    for (var fileNum = 0; fileNum < fileList.length; fileNum++) {
        var v2JSON = jf.readFileSync('../src/main/resources/data/' + fileList[fileNum]);
        var v4JSON = utils.domainsv4[domainNames[domainTypeNum]].mapper.map(v2JSON);

        var fileNameParts = fileList[fileNum].split("_");
        var edipi = fileNameParts[0];
        //use version 4 loinc
        var loinc = utils.domainsv4[domainNames[domainTypeNum]].v4loinc;
        var iter_ext = fileNameParts[2];
