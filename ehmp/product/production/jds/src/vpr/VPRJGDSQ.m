VPRJGDSQ ;SLC/KCM/CJE -- Query for JSON objects (Generic Data Store)
 ;;1.0;JSON DATA STORE;;Sep 01, 2012
 ;
QKEY(KEY,TEMPLATE) ; Return an object given the key (i.e., urn:va:fresh:93EF:10076)
 N VPRDATA,ORDER,GLOBAL,GLOBALJ
...
QCOUNT(CNTNM) ; Return a set of counts across patients
 ...
 S DATA=0,TOPIC=""
 F  S TOPIC=$O(@GLOBALX@("count",CNTNM,TOPIC)) Q:TOPIC=""  D
 . S COUNT=+@GLOBALX@("count",CNTNM,TOPIC)
...
QTALLY(CNTNM) ; Return a set of counts
 ...
 N TOPIC,DATA,COUNT,X
 S DATA=0,TOPIC=""
 F  S TOPIC=$O(@GLOBALX@("tally",CNTNM,TOPIC)) Q:TOPIC=""  D
 . S COUNT=+@GLOBALX@("tally",CNTNM,TOPIC)
 . S X=$S('DATA:"",1:",")_"{""topic"":"""_TOPIC_""",""count"":"_COUNT_"}"
 . S DATA=DATA+1,DATA(DATA)=X
 S X=$$BLDHEAD^VPRJCB(DATA) D STAGE^VPRJCB(X)
 S DATA=0 F  S DATA=$O(DATA(DATA)) Q:'DATA  D STAGE^VPRJCB(DATA(DATA))
 ...
 ;
QINDEX(INDEX,RANGE,ORDER,BAIL,TEMPLATE,FILTER,SKIPLOCK) ; query based on index
