...
QATTR ; return items where attribute value is in range
 ...
 N START,STOP,DIR,SUB,KEY,INST,GLOBAL,GLOBALX
 ; Parsed JSON
...
NXT1() ;
 ...
 Q $O(@GLOBALX@(METHOD,INDEX,SUB(1)),DIR(1))
 ;
NXT2() ;
 I START(2,"collate")="L" S SUB(2)=$O(START(2,"list",SUB(2))) Q SUB(2)
 Q $O(@GLOBALX@(METHOD,INDEX,SUB(1),SUB(2)),DIR(2))
 ;
NXT3() ;
...
ADDONE(KEY,INST,SUB) ; add uid, calculating new sort key if necessary
 ...
 S VPRDATA=VPRDATA+1
 ; case
 I ORDER(0)=0 S:'$D(^||TMP("VPRDATA",$J,KEY)) ^||TMP("VPRDATA",$J,KEY,INST)="" G X1
 I ORDER(0)=1 S:'$D(^||TMP("VPRDATA",$J,SORT(1),KEY)) ^||TMP("VPRDATA",$J,SORT(1),KEY,INST)="" G X1
 I ORDER(0)=2 S:'$D(^||TMP("VPRDATA",$J,SORT(1),SORT(2),KEY)) ^||TMP("VPRDATA",$J,SORT(1),SORT(2),KEY,INST)="" G X1
 I ORDER(0)=3 S:'$D(^||TMP("VPRDATA",$J,SORT(1),SORT(2),SORT(3),KEY)) ^||TMP("VPRDATA",$J,SORT(1),SORT(2),SORT(3),KEY,INST)="" G X1
 I ORDER(0)=4 S:'$D(^||TMP("VPRDATA",$J,SORT(1),SORT(2),SORT(3),SORT(4),KEY)) ^||TMP("VPRDATA",$J,SORT(1),SORT(2),SORT(3),SORT(4),KEY,INST)="" G X1
 I ORDER(0)=5 S:'$D(^||TMP("VPRDATA",$J,SORT(1),SORT(2),SORT(3),SORT(4),SORT(5),KEY)) ^||TMP("VPRDATA",$J,SORT(1),SORT(2),SORT(3),SORT(4),SORT(5),KEY,INST)="" G X1
...
SRTVAL0() ; return value for x
 N X
 S X=$G(@GLOBAL@(KEY,ORDER(LEVEL,CLTN,"srcPath",1)),0) I $D(^(ORDER(LEVEL,CLTN,"srcPath",1),"\s")) S X=X_" "
 Q $S(X]"":X,1:0)
 ;
SRTVAL1() ; return value for x[instance].y
 N I,X
 S I=$G(IARY(ORDER(LEVEL,CLTN,"srcArrays",1,"path"))) Q:'I ""
 S X=$G(@GLOBAL@(KEY,ORDER(LEVEL,CLTN,"srcPath",1),I,ORDER(LEVEL,CLTN,"srcPath",2)),0) I $D(^(ORDER(LEVEL,CLTN,"srcPath",2),"\s")) S X=X_" "
 Q $S(X]"":X,1:0)
 ;
SRTVAL2() ; return value for x[instance].y[instance].z
 ...
 S X=$G(@GLOBAL@(KEY,ORDER(LEVEL,CLTN,"srcPath",1),I,ORDER(LEVEL,CLTN,"srcPath",2),J,ORDER(LEVEL,CLTN,"srcPath",3)),0) I $D(^(ORDER(LEVEL,CLTN,"srcPath",3),"\s")) S X=X_" "
 Q $S(X]"":X,1:0)
 ;
