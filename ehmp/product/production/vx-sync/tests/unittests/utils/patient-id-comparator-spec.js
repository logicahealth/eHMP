...
//--------------------------------------------------------------
// remove this logger before committing to GIT
...
function createEnvironment(vistaClient, config) {
  ...
  //  spyOn(environment.mvi, 'lookup').andCallThrough();
    spyOn(environment.jds, 'getPatientIdentifierByPid').andCallThrough();
    spyOn(environment.publisherRouter, 'publish').andCallThrough();


    return environment;
}
//mock mvi reuslts to return
...
function mviLookupNullResults(patientIdentifier, callback) {
    return callback(null, null);
} 

function mviLookupError(patientIdentifier, callback) {
