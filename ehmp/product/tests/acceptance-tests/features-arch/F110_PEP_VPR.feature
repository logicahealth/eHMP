...
Scenario: Client gets a sensitive patient warning (break the glass scenario) when requesting demographics for a sensitive patient
	...
	When the client requests demographics for that patient "SITE;167" in VPR format
...
Scenario: Client can break the glass after being warned when requesting demographics for a sensitive patient
	...
	When the client requests demographics for that patient "SITE;167" in VPR format
	Then a temporary redirect response is returned
	When the client breaks glass and repeats a request for demographics for that patient "SITE;167" in VPR format
...
Scenario: Non-CPRS user cannot access patient demographics
	Given a patient with "demographics" in multiple VistAs
	When a user "SITE;USER  " with password "USER  !!" requests demographics for that patient "10105V001065" in VPR format
...
Scenario: Patient with different sensitivity levels on multiple VistAs
	Given a patient with "demographics" in multiple VistAs
#	When the client requests demographics for that patient "SITE;18" in VPR format
#	Then a temporary redirect response is returned
#	When the client breaks glass and repeats a request for demographics for that patient "SITE;18" in VPR format
#	Then a successful response is returned
#	When the client requests demographics for that patient "SITE;18" in VPR format
#	Then a temporary redirect response is returned
#	When the client breaks glass and repeats a request for demographics for that patient "SITE;18" in VPR format
