...
Scenario: Client can request paitnet demographics based on ICN or PID in VPR format
  Given a patient with pid "SITE;100603" has been synced through Admin API
  When the client requests demographics from the multiple system for the patient "SITE;100603" in VPR format
...
Scenario: Client can request paitnet demographics based on ICN or PID in VPR format
  Given a patient with pid "10108V420871" has been synced through Admin API
  When the client requests demographics from the multiple system for the patient "SITE;3" in VPR format
  Then a successful response is returned  
  And the VPR results contain
      | field        | value             |
      | pid          | SITE;3            |
