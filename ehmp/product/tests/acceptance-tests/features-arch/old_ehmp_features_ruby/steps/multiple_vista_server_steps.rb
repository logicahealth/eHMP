...
Then(/^the patient with pid "(.*?)" have the same data on both server$/) do |pid|
  ...
  domains.each do |domain|
    response_ve1["data"]["items"][0]['syncStatusByVistaSystemId']['SITE']['domainExpectedTotals']['vital']['total']
    ve1_total = response_ve1["data"]["items"][0]['syncStatusByVistaSystemId']['SITE']['domainExpectedTotals'][domain]['total']
    ve1_count = response_ve1["data"]["items"][0]['syncStatusByVistaSystemId']['SITE']['domainExpectedTotals'][domain]['count']

    ve2_total = response_ve2["data"]["items"][0]['syncStatusByVistaSystemId']['SITE']['domainExpectedTotals'][domain]['total']
    ve2_count = response_ve2["data"]["items"][0]['syncStatusByVistaSystemId']['SITE']['domainExpectedTotals'][domain]['count']
