...
import static gov.va.cpe.vpr.sync.MessageDestinations.*;
...
//    private static final long PATIENT_QUEUE_IDLE_THRESHOLD = 10000;
    ...
    public void sendImportVistaDataExtractItemMsg(final VistaDataChunk item) {
        ...
        try {
            ...
            if(pid==null) {
                getJmsTemplate().convertAndSend("vpr.import", importMsg, new PatientCommandPostProcessor(IMPORT_CHUNK, item.getPatientId(), item.isAutoUpdate()));
            } else {
//                String queueName = "vpr.patient."+pid;
                importMsg.put("pid",pid);
                String queueName = PATIENT_QUEUE;
                getJmsTemplate().convertAndSend(queueName, importMsg, new PatientCommandPostProcessor(IMPORT_CHUNK, item.getPatientId(), item.isAutoUpdate()));
            }
    ...
    public void subscribePatient(String vistaId, PatientDemographics pat) {
        log.debug("SyncService.subscrobePatient(vistaId, pat): Entered method.  vistaId: " + vistaId + "; pat: " + 
    ...
    public void subscribePatient(String vistaId, String pid) {
        log.debug("SyncService.subscrobePatient(vistaId, pid): Entered method.  vistaId: " + vistaId + "; pid: " +pid);
