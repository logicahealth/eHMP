...
module.exports.processJob = function(beanstalkJobId, tubeName, beanstalkJobPayload, callback) {
    ...
    // Call the Handler
    //-----------------
    callHandler(worker, childLogger, doTouch, job, handler, function (error, job) {
...
//---------------------------------------------------------------------------------
function callHandler(worker, logger, touchBack, job, handler, callback) {
    ...
    worker.jobStatusUpdater.startJobStatus(job, function(startJobError) {
        ...
        try {
            handler(logger, job, function(handlerError) {
