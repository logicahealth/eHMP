...
import gov.va.jbpm.tasksservice.utils.TaskLogging;
...
public class TasksController {
	...
	{
		try {
			TaskLogging.debug("Entering getTaskById");
			...
			if (task.getTaskInfoList() != null && !task.getTaskInfoList().isEmpty()) {
				...
				if (taskSummary != null) {
					...
					if (!taskInfo.getVariables().isEmpty()) {
						...
						if (filteredVariableList == null)
							throw new TaskServiceException("TasksController.getTaskById: filteredVariableList cannot be null");
			...
			TaskLogging.error("TasksController.getTaskById: An unexpected condition has happened with rest: " + e.getMessage());
		} catch (TaskServiceException e) {
			//Error was already logged
		} catch (Exception e) {
			TaskLogging.error("TasksController.getTaskById: An unexpected condition has happened: " + e.getMessage());
		}
