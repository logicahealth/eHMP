CREATE OR REPLACE TYPE ACTIVITYDB.LONGARRAY IS TABLE OF NUMBER(19,0);
/
CREATE OR REPLACE TYPE ACTIVITYDB.VARCHARARRAY IS TABLE OF VARCHAR(255);
/
...
CREATE OR REPLACE FUNCTION ACTIVITYDB."SPLIT_STRING_BY_COMMA" (p_string IN VARCHAR2)
RETURN ACTIVITYDB.VARCHARARRAY IS
v_array ACTIVITYDB.VARCHARARRAY;
BEGIN
  v_array := ACTIVITYDB.VARCHARARRAY();
...
--------------------------------------------------------
CREATE OR REPLACE FUNCTION ACTIVITYDB."SPLIT_NUM_STRING_BY_COMMA" (p_string IN VARCHAR2)
RETURN ACTIVITYDB.LONGARRAY IS
v_array ACTIVITYDB.LONGARRAY;
BEGIN
  v_array := ACTIVITYDB.LONGARRAY();
  IF p_string IS NOT NULL THEN
    FOR curs IN (
...
CREATE OR REPLACE FUNCTION ACTIVITYDB."SPLIT_STRING_BY_PIPE" (p_string IN VARCHAR2)
RETURN ACTIVITYDB.VARCHARARRAY IS
v_array ACTIVITYDB.VARCHARARRAY;
BEGIN
  v_array := ACTIVITYDB.VARCHARARRAY();
...
--------------------------------------------------------
CREATE OR REPLACE FUNCTION ACTIVITYDB."PARSE_PRIORITY" (p_priority IN VARCHAR2)
RETURN ACTIVITYDB.LONGARRAY IS
v_array ACTIVITYDB.LONGARRAY;
...
--------------------------------------------------------
CREATE OR REPLACE PACKAGE ACTIVITYDB.TASKS IS
...
--------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY ACTIVITYDB.TASKS IS
--------------------------------------------------------
...
--------------------------------------------------------
  ...
  BEGIN
    v_task_instance_ids := ACTIVITYDB.SPLIT_NUM_STRING_BY_COMMA(p_task_instance_ids);
    OPEN recordset FOR
...
--------------------------------------------------------
  ...
  BEGIN
    v_task_statuses := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_task_statuses);
    v_patient_identifiers := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_patient_identifiers);
    v_priority_array := ACTIVITYDB.PARSE_PRIORITY(p_priority);
    ...
    OPEN recordset FOR
      WITH team_staff_inst_sub AS
      (
        SELECT DISTINCT team_id
        FROM pcmm.team_staff_inst
        WHERE staff_ien = p_staff_id AND stationnumber like v_station_number_prefix
      )
      SELECT DISTINCT ti.id as taskId,
            ...
            ti.before_earliest_date as beforeEarliestDate,
            ...
            WHERE (tr.team IN (SELECT team_id FROM team_staff_inst_sub))
...
--------------------------------------------------------
  ...
  open recordset for
      WITH team_staff_inst_sub AS
      (
        SELECT /*+ materialize */ team_id, pcm_std_team_focus_code, pcm_std_team_care_type_id, pcm_std_team_role_id
        FROM pcmm.team_staff_inst
        WHERE staff_ien = p_staff_id AND stationnumber like v_station_number_prefix
      )
      SELECT DISTINCT ti.id as taskId,
            ...
            WHERE
             ((tr.userid = p_user_id   -- routed directly to me
                    ...
                    OR (tr.userid IS NULL  --not routed to another user
                      ...
                      AND ((tr.team IS NULL) OR (tr.team IN (SELECT UNIQUE team_id FROM team_staff_inst_sub)))
                      AND ((tr.facility IS NULL) OR (tr.facility IS NOT NULL AND tr.facility = p_station_number))
                      AND ((tr.teamfocus IS NULL) OR (tr.teamfocus IN (SELECT UNIQUE pcm_std_team_focus_code FROM team_staff_inst_sub)))
                      AND ((tr.teamtype IS NULL) OR (tr.teamtype IN (SELECT UNIQUE pcm_std_team_care_type_id FROM team_staff_inst_sub)))
                      AND ((tr.teamrole is null) OR (tr.teamrole IN (SELECT UNIQUE pcm_std_team_role_id FROM team_staff_inst_sub)))
                    ))
                  ...
                  AND ((p_task_statuses IS NULL AND ti.statusId IN (1,2,3))
                      OR (p_task_statuses IS NOT NULL AND
                          ...
                          OR (tsl.status IN (SELECT TO_CHAR(regexp_substr(p_task_statuses,'[^,]+', 1, level)) from dual
                            CONNECT BY regexp_substr(p_task_statuses, '[^,]+', 1, level) is not null
                          ))))
                  )
                  AND (p_process_instance_id IS NULL OR ti.processInstanceId = p_process_instance_id)
...
--------------------------------------------------------
  ...
  BEGIN
    v_patient_identifiers := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_patient_identifiers);
    v_activity_states := ACTIVITYDB.SPLIT_STRING_BY_PIPE(p_activity_states);
    ...
    OPEN recordset FOR
      SELECT lti.latestTask as taskId,
            ...
            WHERE psl.status = 'Active'
                  ...
                  AND (pi.PID IN (SELECT TO_CHAR(column_value) FROM TABLE(v_patient_identifiers))
                       OR pi.ICN IN (SELECT TO_CHAR(column_value) FROM TABLE(v_patient_identifiers)) )
...
GRANT EXECUTE ON ACTIVITYDB.TASKS TO ehmp_rw_role ;
/
--------------------------------------------------------
--  DDL for package PCMMDATA
--------------------------------------------------------
CREATE OR REPLACE PACKAGE ACTIVITYDB.PCMMDATA IS
...
--------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY ACTIVITYDB.PCMMDATA IS
--------------------------------------------------------
...
GRANT EXECUTE ON ACTIVITYDB.PCMMDATA TO ehmp_rw_role ;
/

--------------------------------------------------------
--  DDL for package ACTIVITIES
--------------------------------------------------------
CREATE OR REPLACE PACKAGE ACTIVITYDB.ACTIVITIES IS
...
--------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY ACTIVITYDB.ACTIVITIES IS
...
--------------------------------------------------------
    ...
    BEGIN
        OPEN recordset FOR
            ...
            NVL(AM_PROCESSINSTANCE.PID, AM_PROCESSINSTANCE.ICN) as pid,
...
--------------------------------------------------------
    ...
    BEGIN
        v_team_ids := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_team_ids);
        OPEN recordset FOR
...
--------------------------------------------------------
    ...
    BEGIN
        v_patient_ids := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_patient_ids);
        v_team_ids := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_team_ids);
        v_team_focus_ids := ACTIVITYDB.SPLIT_STRING_BY_COMMA(p_team_focus_ids);
        OPEN recordset FOR
                ...
                (
                    ...
                    (
                        ...
                        (
                            ...
                            LEFT OUTER JOIN PCMM.PCM_STD_TEAM_FOCUS_MV tf on p.TEAMFOCUS = tf.CODE
...
GRANT EXECUTE ON ACTIVITYDB.ACTIVITIES TO ehmp_rw_role ;
/
