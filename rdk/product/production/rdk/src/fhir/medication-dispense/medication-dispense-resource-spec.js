...
var medDispense = require('./medication-dispense-resource');
...
describe('Medication Dispense Conformance Statement', function() {
    ...
    var testConformance = medDispenseIn.conformanceData;
    var builtConformance = medDispense.createConformanceData();

    it('verifies that a generated conformance statement exists', function() {
        expect(builtConformance).not.to.be.undefined();
    });

    describe('Medication Dispense Conformance Details', function() {
        ...
        it('verifies conformance search parameter is valid', function() {
...
describe('Document FHIR Resource', function() {
    ...
    _.each(vprDocuments, function(vprDoc) {
        it('verifies that each VPR Medication Dispense Resource has a coresponding FHIR Medication Dispense Resource in the collection with the same uid', function() {
            ...
            var siteHash = fhirUtils.getSiteHash(vprDoc.uid);
            ...
            describe('found FHIR Medication Dispense coresponds to the original VPR Medication Dispense Resource - ' + vprDoc.uid, function() {
                ...
                describe('Medication Dispense', function() {
                    ...
                    describe('Contained Resources', function() {
                        ...
                        it('verifies that the start and stop from VPR Medication Dispense Resource coresponds to the one from the FHIR Medication Dispense Resource', function() {
                            if (dispenseMedPrescription.dosageInstruction && dispenseMedPrescription.dosageInstruction[0]) {
                                expect(dispenseMedPrescription.dosageInstruction[0].timing.period.start).to.eql(fhirUtils.convertToFhirDateTime(vprDoc.dosages[0].start, siteHash));
                                expect(dispenseMedPrescription.dosageInstruction[0].timing.period.end).to.eql(fhirUtils.convertToFhirDateTime(vprDoc.dosages[0].stop, siteHash));
                            }
                        });
                ...
                describe('Contained Resources', function() {
                    ...
                    it('verifies that the ordered from VPR Medication Dispense Resource coresponds to the one from the FHIR Medication Dispense Resource', function() {
                        expect(medPrescription.dateWritten).to.eql(fhirUtils.convertToFhirDateTime(vprDoc.orders[0].ordered, siteHash));
                    });
