...
var req = {
    ...
    originalUrl: '/fhir/ReferralRequest?subject.identifier=SITE;253'
};
...
describe('ReferralRequest FHIR conversion methods', function() {
    ...
    it('bundle results correctly', function() {
        ...
        expect(fhirBundle.link[0].url).to.equal('http://localhost:PORT/fhir/ReferralRequest?subject.identifier=SITE;253');
    ...
    _.each(vprConsultations, function(vprItem) {
        describe(':: ReferralRequest ::', function() {
            ...
            it('sets dateSent correctly', function() {
                expect(fhirItem.dateSent).to.equal(fhirUtils.convertToFhirDateTime(vprItem.dateTime, fhirUtils.getSiteHash(vprItem.uid)));
            });
...
describe('Composition FHIR conformance', function() {
    ...
    var conformanceData = referralRequestResource.createConformanceData();

    it('conformance data is returned', function() {
        expect(conformanceData.type).to.equal('referralrequest');
        expect(conformanceData.profile.reference).to.equal('http://www.hl7.org/FHIR/2015May/referralrequest.html');
