...
//TODO: Examine the need to handle multiple patient identifier types.  It is currently assumed that all
 ...
 *
 * @apiParam {Number} [id] Patientlist id (24 digit HEX number), id has
...
module.exports.getPatientList = function(req, res) {
    ...
    if (match.name || match._id) {
        ...
        db.collection(patCollection).findOne(match, function(err, result) {
...
module.exports.postPatientlist = function(req, res) {
    ...
    db.collection(patCollection).findOne(match, function(err, result) {
        ...
        db.collection(patCollection).insert(doc, function(err, result) {
...
module.exports.deletePatientlist = function(req, res) {
    ...
    db.collection(patCollection).remove(match, function(err, result) {
        var message = (err === null) ? result : err;
...
module.exports.addPatient = function(req, res) {
    ...
    db.collection(patCollection).findOne(match, function(err, result) {
        ...
        db.collection(patCollection).update({
            _id: result._id
 ...
 * @apiParam {
 ...
 value the actual Patient ID value to be found
 *
 * @apiSuccess (Success 200) {json} data boolean value
 * @apiSuccessExample Success-Response: HTTP/1.1 200 OK
 ...
 *
 */
module.exports.checkPatientMembershipStatus = function(req, res) {
    if (!isCDSMongoServerAvailable) {
        return res.status(rdk.httpstatus.service_unavailable).rdkSend('CDS persistence store is unavailable.');
    }
    ...
    http.get(options, function(error, response, result) {
        ...
        db.collection(patCollection).findOne(matchStatement, function(err, result) {
            if (err) {
                return res.status(rdk.httpstatus.internal_server_error).rdkSend(err);
            }
            var isMember = dd(result)('patients').exists && !_.isEmpty(result.patients);
...
module.exports.copyPatientlist = function(req, res) {
    ...
    db.collection(patCollection).findOne(match, function(err, patlst) {
        ...
        delete match._id;
        match.name = newname;
        db.collection(patCollection).findOne(match, function(err, test) {
            ...
            patlst.name = newname;
            delete patlst._id;
            db.collection(patCollection).insert(patlst, function(err, result) {
                message = (err === null) ? result : err;
                if (err) {
                    status = rdk.httpstatus.internal_server_error;
                }
