...
function _update(req, res, data) {
    ...
    pjds.put(req, res, pjdsOptions, function(error, result, headers) {
        if (error) {
            req.logger.error('QuickOrder update (uid:' + data.uid + '): ', error);
...
function _search(req, res, name, siteId, userId, callback) {
...
function save(req, res, isNew) {
    ...
    checkNameCollission(req, res, quickOrder.name, quickOrder.scope, quickOrder.siteId, quickOrder.createdBy, isNew ? null : quickOrder.uid, function(error, hasNameCollision) {
        if (error) {
            req.logger.error('QuickOrder ' + isNew ? 'create' : 'update' + ': ', error);
...
module.exports.search = function(req, res) {
    _search(req, res, req.param('name'), req.session.user.site, req.session.user.uid, function(error, result) {
        if (error) {
...
function getQuickOrder(req, res, uid, callback) {
    var pjdsOptions = {
        store: 'quickorder',
        key: uid
    };
...
module.exports.getQuickOrder = getQuickOrder;
