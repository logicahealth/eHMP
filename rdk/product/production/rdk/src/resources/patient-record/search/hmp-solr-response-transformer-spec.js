...
var addSynonymsToResponse = hmpSolrResponseTransformer._addSynonymsToResponse;
var removeEscapeChars = hmpSolrResponseTransformer._removeEscapeChars;

describe('detect solr grouping', function() {
    it('should detect when grouping is on', function() {
        var specializedSolrResult = {
            grouped: {}
        };
...
describe('transform solr highlighting to hmp object', function() {
    it('should put solr highlighting into an hmp object', function() {
        ...
        var hmpEmultatedResponseObject = buildResponseObjectSkeleton({
            query: 'foobar'
        });
...
describe('get synonyms from solr response', function() {
    it('data should contain synonyms array', function() {
        var solrResponse = {
            responseHeader: {
                status: 0,
                QTime: 0,
            ...
            debug: {
                rawquerystring: 'blood sugar',
                querystring: 'blood sugar',
                expandedSynonyms: [
                    '\"blood sugar\"',
                    '\"dextrose\"',
                    '\"glucose=> other\"',
        ...
        var rdkResult = ['blood sugar', 'dextrose', 'glucose=> other', 'blood', 'sugar'];

        expect(JSON.stringify(hmpEmultatedResponseObject.data.synonyms)).to.equal(JSON.stringify(rdkResult));
    });

    it('should remove any duplicate synonym values', function() {
        var solrResponse = {
            responseHeader: {
                status: 0,
                QTime: 0,
            ...
            debug: {
                ...
                querystring: 'blood sugar',
                expandedSynonyms: [
                    '\"blood sugar\" \"heavy meromyosin\"',
                    '\"blood sugar\" \"hexamethyl melamine\"',
                    '\"blood sugar\" \"hexamethyl-melamine\"',
                    '\"blood sugar\" \"hexamethylmelamine\"',
                    '\"blood sugar\" \"hexamethylomelamine\"',
                    '\"blood sugar\" \"hidden markov model\"',
                    '\"sugar\" \"heavy meromyosin\"',
                    '\"sugar\" \"hexamethyl melamine\"',
                    '\"sugar\" \"hexamethyl-melamine\"',
                    '\"sugar\" \"hexamethylmelamine\"',
                    '\"sugar\" \"hexamethylomelamine\"',
                    '\"sugar\" \"hidden markov model\"'
                ],
                mainQueryParser: [
                    'QParser',
                    'ExtendedDismaxQParser',
        ...
        var rdkResult = ['blood sugar', 'heavy meromyosin', 'hexamethyl melamine', 'hexamethyl-melamine', 'hexamethylmelamine', 'hexamethylomelamine',
            'hidden markov model', 'sugar'
        ];

        expect(JSON.stringify(hmpEmultatedResponseObject.data.synonyms)).to.equal(JSON.stringify(rdkResult));
    });

    it(' keep escape chars if synonym has any in it', function() {
        var solrResponse = {
            responseHeader: {
                status: 0,
                QTime: 0,
...
describe('check escaping ', function() {
    it(' remove escap chars ', function() {
        var item = '\"singlet oxygen\"';

        var result = removeEscapeChars(item);
        var rdkResult = 'singlet oxygen';
        expect(result).to.equal(rdkResult);
    });

    it(' rdk result should keep if synonym has any escape chars in it', function() {
        var item = '\"2\"-terpyridine\"';

        var result = removeEscapeChars(item);
        var rdkResult = '2\"-terpyridine';
        expect(result).to.equal(rdkResult);
    });
});
...
function generateFakeSolrResponse(domain, group_field) {
    ...
    if (group_field) {
        solrResult.grouped = {};
        solrResult.grouped[group_field] = {
            matches: Math.random() * 10 + 10
        };
    } else {
        solrResult.response = {};
