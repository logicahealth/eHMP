...
var rules = require('../pdp/policy-rules').rules;
...
module.exports = function(req, res, callback) {
    req.logger.debug('PEP: processing pep policy request');
    req.audit.sensitive = 'false';
    getPatient(req, function(err, result) {
        if (err) {
            return callback(err);
        }
        var userPermission = userPolicyBuilder(req, result);
        rulesEngine.executeRules(rules, userPermission, function(results) {
            req.logger.debug('PEP: ' + results.code + ' pep response received.');
