...
function encryptSig (input) {
    ...
    var pad = {assocKey: a, assocVal: CIPHER_PAD[a], idKey: i, idVal: CIPHER_PAD[i]};

    var encryptedValue = input.split('').reduce(function(acc, item) {
        var pos = pad.assocVal.indexOf(item);
        return acc + (pos === -1 ? item : pad.idVal.charAt(pos));
    }, '');
    ...
    return wrap(pad.assocKey) + encryptedValue + wrap(pad.idKey);
}
