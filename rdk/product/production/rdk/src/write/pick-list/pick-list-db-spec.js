...
describe('pick-list-db', function() {
    var testData = {
        one: {
            data: 1
        },
    ...
    describe('verify database operations', function() {
        ...
        it('loads an entry into the database', function(done) {
            dbList._loadPickList(fakeAppObject, fakeSiteConfig, {
                'site': fakeSiteHash,
                'pickList': fakePickListName
            }, 'pick-list-db-spec', function(err, result) {
        ...
        it('overwrites duplicate data', function(done) {
            ...
            dbList._updateDatabase(log, {
                'site': fakeSiteHash,
                'pickList': fakePickListName
            }, newTimeStamp, testData, function(err, result) {
                expect(err).to.be.falsy();
                expect(result).to.be.truthy();
                expect(result).to.eql(testData);
        ...
        it('refreshes expired data', function(done) {
            dbList.retrievePickList(fakeAppObject, fakeSiteConfig, {
                'site': fakeSiteHash,
                'pickList': fakePickListName
            }, null, 'pick-list-db-spec', 0, function(err, result) {
                expect(err).to.be.falsy();
                expect(result).to.be.truthy();
                expect(result).to.eql(testData);
        ...
        it('doesn\'t refresh unexpired data', function(done) {
            dbList.retrievePickList(fakeAppObject, fakeSiteConfig, {
                'site': fakeSiteHash,
                'pickList': fakePickListName
            }, null, 'pick-list-db-spec', 10, function(err, result) {
                expect(err).to.be.falsy();
                expect(result).to.be.truthy();
                expect(result).to.eql(testData);
