...
function problemMatch(logger, model, problem) {
    ...
    if (_.isUndefined(model) || _.isUndefined(problem) || _.isUndefined(problem.entered) || _.isUndefined(problem.updated) || _.isUndefined(problem.problemText) || _.isUndefined(problem.providerUid)) {
        logger.error({
            model: model,
            problem: problem
        });
    ...
    if (!_.endsWith(problem.providerUid, model.responsibleProviderIEN.toString())) {
        logger.debug({
            providerUid: problem.providerUid,
            responsibleProviderIEN: model.responsibleProviderIEN
        });
        return false;
    }
    ...
    async.waterfall([
            ...
            function getProblemJSON(input, callback) {
                ...
                if (input === BAD_ADD_RESPONSE) {
                    logger.error({
                        errorFromFirst: input
                    });
                    return callback(BAD_ADD_RESPONSE);
                }

                var parameters = {
                    '"patientId"': model.patientIEN,
                    '"domain"': 'problem'
                };
...
function add(writebackContext, callback) {
    ...
    logger.debug({
        rpcParameters: writebackContext.model.rpcParameters
    });
