...
Scenario: Request to unsync a patient using secondary siteid
	...
	When the client requests that the patient with pid "SITE;227" be cleared through the RDK API
...
Scenario: Request to unsync a patient using secondary siteid
	...
	When the client requests that the patient with pid "SITE;227" be cleared through the RDK API
	Then a successful response is returned
	And the patient with pid "SITE;227" is cleared throught the RDK API within 120 seconds
