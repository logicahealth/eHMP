...
Scenario: Manually check that accessed records are flagged as sensitive for certain sites - ensure sensitivity and site code are in each audit entry
    ...
    Then to manually confirm the entry is there ssh PW@IP        
    And the password is PW
    Then cd /tmp
    Then tail audit.log
...
Scenario: Manually check that special accounts are denoted with XUPROG and/or XUPROGMODE keys
    ...
    Then to manually confirm the a GET request entry is there and that the keys are present by doing ssh PW@IP        
    And the password is PW
    Then cd /tmp
    Then tail audit.log
...
Scenario: A request for resource directory is audited
    ...
    When user performs ssh PW@IP        
    And user enters "PW" as the password
...
Scenario: A request for a patient's record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/patient?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/patient?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | patient                                                          |
...
Scenario: A request for a patient's meds record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/med?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/med?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's consults record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/consult?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/consult?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | consult                                                          |
...
Scenario: A request for a patient's vitals record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/vital?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/vital?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's problems record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/problem?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/problem?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | problem                                                         |
...
Scenario: A request for a patient's allergies record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/allergy?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/allergy?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | allergy                                                          |
...
Scenario: A request for a patient's orders record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/order?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/order?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | order                                                          |
...
Scenario: A request for a patient's treament record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/treatment?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/treatment?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's procedures record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/procedure?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/procedure?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's lab record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/lab?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/lab?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's images record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/image?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/image?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | procedure                                                          |
...
Scenario: A request for a patient's surgeries record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/surgery?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/surgery?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | procedure                                                         |
...
Scenario: A request for a patient's documents is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/document?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/document?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | document                                                          |
...
Scenario: A request for a patient's mh is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/mh?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/mh?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's immunization's is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/immunization?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/immunization?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's pov is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/pov?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/pov?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's dermatology record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/skin?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/skin?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | skintest                                                          |
...
Scenario: A request for a patient's exams is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/exam?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/exam?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | exam                                                        |
...
Scenario: A request for a patient's cpt is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/cpt?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/cpt?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's education record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/education?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/education?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's factor is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/factor?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/factor?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's appointments is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/appointment?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/appointment?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's visits is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/visit?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/visit?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | encounter                                                          |
...
Scenario: A request for a patient's ptf record is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/ptf?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/ptf?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's newsfeed is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/newsfeed?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/newsfeed?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's document view is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/document-view?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/document-view?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's vlerdoc view is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/vlerdocument?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/vlerdocument?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient's parent documents is audited
    Given a postman request of http://IP             /resource/patientrecord/domain/parent-documents?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/domain/parent-documents?pid=SITE;100022        |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient record suggested search is audited
    Given a postman request of http://IP             /resource/patientrecord/search/suggest?pid=SITE;100022&query=true as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/search/suggest?pid=SITE;100022&query=true HTTP/1.1       |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | logCategory                      | RETRIEVE                                                      |
...
Scenario: A request for a patient labs by order search is audited
    Given a postman request of http://IP             /resource/patientrecord/labsbyorder?pid=SITE;100022&orderUid=urn:va:order:SITE:253:15477 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
...
Scenario: A request for a patient cwad search is audited
    Given a postman request of http://IP             /resource/patientrecord/cwad?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/cwad?pid=SITE;100022 HTTP/1.1      |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | logCategory                      | RETRIEVE                                                      |
...
Scenario: A request for a patient's timeline is audited
    Given a postman request of http://IP             /resource/patientrecord/timeline?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET /resource/patientrecord/timeline?pid=SITE;100022 HTTP/1.1      |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for a patient complex note is audited
    Given a postman request of http://IP             /resource/patientrecord/complexnote?pid=SITE;100022&uid=urn:va:combat-vet:N as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | http://IP             /resource/patientrecord/complexnote?pid=SITE;100022&uid=urn:va:combat-vet:N     |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | logCategory                      | RETRIEVE                                                      |
...
Scenario: A request for a synch of expired patient data is audited
    Given a postman request of http://IP             /resource/sync/expire?pid=SITE;3&vistaId="DOD" as a POST is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | POST /resource/sync/expire?pid=SITE;3&vistaId=%22DOD%22 HTTP/1.1   |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | logCategory                      | EXPIRE                                                      |
...
Scenario: A request for a UID is audited
    Given a postman request of http://IP             /resource/patientrecord/uid?pid=SITE;100022&uid=urn:va:mst:U as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/patientrecord/uid?pid=SITE;100022&uid=urn:va:mst:U  |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | status                           | 404                                                          |
...
Scenario: A request for labs by order is audited
    Given a postman request of http://IP             /resource/patientrecord/labsbytype?pid=SITE;100022&typeName=HOSPITALIZATION as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/patientrecord/labsbytype?pid=SITE;100022&typeName=HOSPITALIZATION |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for labs by panel is audited
    Given a postman request of http://IP             /resource/patientrecord/labpanels?pid=SITE;100022 as a GET is made
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/patientrecord/labpanels?pid=SITE;100022 |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for a lab search is audited
    Given a postman request of http://IP             /resource/searchbytype/domain/lab?pid=SITE;100022&type=influenza
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/searchbytype/domain/lab?pid=SITE;100022&type=influenza       |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | lab                                                         |
...
Scenario: A request for an immunization search is audited
    Given a postman request of http://IP             /resource/searchbytype/domain/immunization?pid=SITE;100022&type=influenza
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/searchbytype/domain/immunization?pid=SITE;100022&type=influenza       |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | immunization                                                        |
...
Scenario: A request for a vital search is audited
    Given a postman request of http://IP             /resource/searchbytype/domain/vital?pid=SITE;100022&type=pulse
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/searchbytype/domain/vital?pid=SITE;100022&type=pulse       |
      | sensitive                        | false                                                         |
      | patientID                        | SITE;100022                                                   |
      | sitecode                         | SITE                                                          |
      | dataDomain                       | vital                                                         |
...
Scenario: A request for resource global timeline is audited
    Given a postman request of http://IP             /resource/globaltimeline?pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/globaltimeline?pid=SITE;100022     |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for closest vitals is audited
    Given a postman request of http://IP             /resource/vitals/closest?pid=SITE;100022&dfn=3&type=SBP
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/vitals/closest?pid=SITE;100022&dfn=3&type=SBP    |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for vitals qualifiers is audited
    Given a postman request of http://IP             /resource/vitals/qualifiers?pid=SITE;100022&dfn=3&type=WT,HT,,T
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/vitals/qualifiers?pid=SITE;100022&dfn=3&type=WT,HT,,T   |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for all vitals is audited
    Given a postman request of http://IP             /resource/vitals/all?pid=SITE;100022&dfn=3&start=3010101&end=3141030
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/vitals/all?pid=SITE;100022&dfn=3&start=3010101&end=3141030   |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for visits appointments is audited
    Given a postman request of http://IP             /resource/visits/appointments?pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/admissions?pid=SITE;100022  |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
...
Scenario: A request for CDS advice list is audited
    Given a postman request of http://IP             /resource/cds_advice/list?use=FamilyMedicine&pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/cds_advice/list?use=FamilyMedicine&pid=SITE;100022  |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | dataDomain                       | CDS                                                           |
      | logCategory                      | ADVICE                                                        |
...
Scenario: A request for CDS advice detail is audited
    Given a postman request of http://IP             /resource/cds_advice/detail?use=FamilyMedicine&pid=SITE;100022&adviceid=500074
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/cds_advice/detail?use=FamilyMedicine&pid=SITE;100022&adviceid=500074  |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | dataDomain                       | CDS                                                           |
      | logCategory                      | ADVICE                                                        |
...
Scenario: A request for orders detail is audited
    Given a postman request of http://IP             /resource/order/detail?orderId=2157546463&dfn=3&pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/order/detail?orderId=2157546463&dfn=3&pid=SITE;100022            |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for patient record text search is audited
    Given a postman request of http://IP             /resource/patientrecord/search/text?pid=SITE;100022&query="Text searching for"
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/patientrecord/search/text?pid=SITE;100022&query="something here"            |
      | patientPidList                   | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | logCategory                      | SEARCH                                                        |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A request for patient record detail document search is audited
    Given a postman request of http://IP             /resource/patientrecord/search/detail/document?pid=SITE;100022&query="Text searching for"&group_value=4&group_field=5
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/patientrecord/search/detail/document?pid=SITE;100022&query="Something something"&group_value=4&group_field=5            |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 200                                                           |
...
Scenario: A patient sync load is audited
    Given a postman request of http://IP             /resource/sync/load?pid=SITE:100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/sync/load?pid=SITE:100022            |
      | patientID                        | SITE;100022                                                   |
      | logCategory                      | SYNC                                                          |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
...
Scenario: A vitals writeback save is audited
    Given a postman request of http://IP             /resource/writeback/vitals/save?dfn=SITE;3&duz=10000000224&locIEN=67&vitals=[{"fileIEN":"19","reading":"45"}]&pid=SITE;3
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | POST http://IP             /resource/writeback/vitals/save?dfn=SITE;3&duz=10000000224&locIEN=67&vitals=[{"fileIEN":"19","reading":"45"}]&pid=SITE;3           |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 500                                                           |
...
Scenario: An allergy writeback save is audited
    Given a postman request of http://IP             /resource/writeback/allergy/save?dfn=SITE;3&duz=10000000224&locIEN=67&allergy=[{"fileIEN":"19","reading":"45"}]&pid=SITE;3
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | POST http://IP             /resource/writeback/allergy/save?dfn=SITE;3&duz=10000000224&locIEN=67&vitals=[{"fileIEN":"19","reading":"45"}]&pid=SITE;3           |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 500                                                           |
...
Scenario: A request for a sync status is audited
    Given a postman request of http://IP             /resource/sync/status?pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/sync/status?pid=SITE;100022     |
      | patientID                        | SITE;100022                                                   |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 404                                                           |
...
Scenario: A request for datasync status is audited
    Given a postman request of http://IP             /resource/sync/datastatus?pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/sync/datastatus?pid=SITE;100022     |
      | logCategory                      | SYNC                                                          |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 404                                                           |
...
Scenario: A request for operational status sync is audited
    Given a postman request of http://IP             /resource/sync/operationalstatus?pid=SITE;100022
    When user performs ssh PW@IP        
    And user enters "PW" as the password
    And user cd traverses to the /tmp directory by performing "cd /tmp"
    And user views the audit log by performing the action "tail audit.log"
    Then the last entry in the audit log contains
      ...
      | authuser                         | SITE:10000000227                                              |
      | date                             | IS_FORMATTED_DATE                                             |
      | request                          | GET http://IP             /resource/sync/operationalstatus?pid=SITE;100022    |
      | logCategory                      | SYNC                                                          |
      | sensitive                        | false                                                         |
      | sitecode                         | SITE                                                          |
      | status                           | 404                                                           |
