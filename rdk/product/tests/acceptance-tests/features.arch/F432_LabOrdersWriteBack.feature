...
Scenario: Creating a lab order through RDK
 	Given a patient with pid "SITE;100472" has been synced through the RDK API
...
@discontinueorder
	Scenario: Discontinuing an existing/unsigned lab order through RDK
 	Given a patient with pid "SITE;100472" has been synced through the RDK API
  	...
  	Then a successful response is returned
    And the client clears patient with pid "SITE;100472" info from vxsync
    Then the client syncs the patient with pid "SITE;100472" in vxsync
