...
Scenario: Create Endpoint for ORWDRA32 IMTYPSEL (RADIOLOGY)
  When the client requests picklist with the parameters for "radiology-imaging-types" with the user "SITE;USER  "
      | paramter name | value                                   |
...
Scenario: Create Endpoint for ORWDRA32 DEF (RADIOLOGY)
  When the client requests picklist with the parameters for "radiology-dialog-default" with the user "SITE;USER  "
      | paramter name | value                    |
...
When the client requests picklist with the parameters for "lab-all-samples" with the user "SITE;USER  "
      | paramter name | value                                 |
Then a successful response is returned
And the picklist result contains
      | field             | value       |
...
When the client requests picklist with the parameters for "lab-order-specimens" with the user "SITE;USER  "
      | paramter name | value               |
  Then a successful response is returned
  And the picklist result contains
      | field | value                |
...
When the client requests picklist with the parameters for "lab-order-dialog-def" with the user "SITE;USER  "
      | paramter name | value                |
      | location  | 240           |
  Then a successful response is returned
  And the picklist result contains
...
When the client requests picklist with the parameters for "lab-times-available" with the user "SITE;USER  "
      | paramter name | value               |
      | date          | 20312015            |
      | locationUid      | urn:va:location:SITE:1 |
  Then a successful response is returned
  And the picklist result contains
      | field   | value         |
      | time    |  0930         |
...
When the client requests picklist with the parameters for "lab-time-valid-immediate-collect-time" with the user "SITE;USER  "
      | paramter name | value                                 |
      | time          | 3151127.151                           |
Then a successful response is returned
...
When the client requests picklist with the parameters for "lab-collect-times" with the user "SITE;USER  "
      | paramter name | value             |
  Then a successful response is returned
  And the picklist result contains
      | field      | value                               |
...
When the client requests a picklist with the parameters for "new-persons" with the user "SITE;USER  "
      | paramter name | value       |
      | site          | SITE        |
  Then a successful response is returned
...
Scenario: Create Endpoint for allergies match (ORWDAL32 ALLERGY MATCH)
  When the client requests picklist with the parameters for "allergies-match" with the user "SITE;USER  "
      | paramter name | value                          |
...
Scenario: Create Endpoint for allergies-symptoms-top-ten (ORWDAL32 DEF)
  When the client requests picklist with the parameters for "allergies-symptoms-top-ten" with the user "SITE;USER  "
      | paramter name | value                          |
  Then a successful response is returned
  And the picklist result contains
      | field        | value           |
...
Scenario: Create Endpoint for allergies symptoms (ORWDAL32 SYMPTOMS)
  When the client requests picklist with the parameters for "allergies-symptoms" with the user "SITE;USER  "
      | paramter name | value              |
      | searchString  | hives              |
  Then a successful response is returned
  And the picklist result contains
...
Scenario: Create Endpoint for allergies symptoms all with top ten (ORWDAL32 DEF and ORWDAL32 SYMPTOMS )
  When the client requests picklist with the parameters for "allergies-symptoms-all-with-top-ten" with the user "SITE;USER  "
      | paramter name | value                               |
  Then a successful response is returned
  And the picklist result contains
      | field        | value           |
...
Scenario: Create Endpoint for problems-lexicon-lookup (ORQQPL4 LEX)
  When the client requests picklist with the parameters for "problems-lexicon-lookup" with the user "SITE;USER  "
      | paramter name | value                   |
      | searchString  | ABC                     |
  Then a successful response is returned
  And the picklist result contains
...
Scenario: Get list of vitals qualifiers/categories
  When the client requests picklist with the parameters for "vitals" with the user "SITE;USER  "
      | paramter name | value |
...
Scenario: Get List of printer devices
  When the client requests picklist with the parameters for "printer-devices" with the user "SITE;USER  "
      | paramter name | value |
...
Scenario: Returns a list of Note Titles
  When the client requests picklist with the parameters for "progress-notes-titles" with the user "SITE;USER  "
      | paramter name | value |
      | class         | 3 |
...
Scenario: Returns the flags for a Progress Note Title
  When the client requests picklist with the parameters for "progress-notes-titles-flags" with the user "SITE;USER  "
      | paramter name | value                       |
      | ien           | 1354                       |
      | searchString  | ACROMEGALY                  |
...
Scenario: Create endpoint for ORWDX ORDITM (lab order types)
  When the client requests picklist with the parameters for "lab-order-orderable-items" with the user "SITE;USER  "
