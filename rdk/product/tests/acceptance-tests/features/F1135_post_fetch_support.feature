...
Scenario: Add POST fetch support to patient record resource
  ...
  When the client posts a document-view request with parameters
  | parameter | value  |
  | pid       | 9E7A;8 |
  | filter    | or(between(referenceDateTime,"20140701","20170417235959"),between(dateTime,"20140701","20170417235959")),not(and(in(kind,["Consult","Imaging","Procedure"]),ne(statusName,"COMPLETE")))       |
  Then a successful response is returned
  And the documents FETCH results are the same as the GET results
...
Scenario: Add POST fetch support to patient record resource
  Given a patient with pid "9E7A;8" has been synced through the RDK API
  When the client posts a document-view request with a query parameter for patient "9E7A;8" and a body parameter for "9E7A;3"
  | parameter | value  |
  | filter    | or(between(referenceDateTime,"20140701","20170417235959"),between(dateTime,"20140701","20170417235959")),not(and(in(kind,["Consult","Imaging","Procedure"]),ne(statusName,"COMPLETE")))       |
  Then a bad request response is returned
  And the response message is "Body pid must match query parameter pid"
