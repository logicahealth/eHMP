...
Scenario: 1. Parameter returnSynonyms is omitted (default), resulting in no synonyms element returned
  When the client searches text for pid "SITE;253" and query "cdi"
...
Scenario: 2. Parameter returnSynonyms is false, resulting in no synonyms element returned
  When the client searches text for pid "SITE;253" and query "cdi" with returnSynonyms "false"
  Then a successful response is returned
  And the response does not contain array "synonyms"

@F1207-1.3
Scenario: 3. Parameter returnSynonyms is non-boolean, resulting in 4xx error
  When the client searches text for pid "SITE;253" and query "cdi" with returnSynonyms "xxxx"
  Then a bad request response is returned

@F1207-1.4
Scenario: 4. No synonyms found returns just original term
  When the client searches text for pid "SITE;253" and query "chocolate" with returnSynonyms "true"
...
Scenario: 5. One search term returning just one synonym
  When the client searches text for pid "SITE;253" and query "(1)o(2)" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains array "synonyms"
    | synonym        |
    | singlet oxygen |
...
Scenario: 6. One search term returning many synonyms (large list)
  When the client searches text for pid "SITE;253" and query "ct" with returnSynonyms "true"
  Then a successful response is returned
...
Scenario: 7. Multiple search terms to verify duplicates are removed on synonym list
  When the client searches text for pid "SITE;253" and query "bs" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains only 25 synonyms

  When the client searches text for pid "SITE;253" and query "glucose" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains only 3 synonyms

  When the client searches text for pid "SITE;253" and query "glucose bs" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains only 27 synonyms
  And the response contains array "synonyms"
    | synonym                  |
    | bachelor of science    |
    | bachelor of surgery    |
...
Scenario: 8. Search term where synonym file has duplicate but one is quoted and other is not
  When the client searches text for pid "SITE;253" and query "blood sugar" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains array "synonyms"
    | synonym         |
    | blood           |
...
Scenario: 9. Multiple terms where one has a synonym and the other does not
  When the client searches text for pid "SITE;253" and query "chocolate (1)o(2)" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains array "synonyms"
    | synonym                   |
    | chocolate                 |
...
Scenario: 10. Multiple terms where at least one has synonyms, changing search term order does not change list
  When the client searches text for pid "SITE;253" and query "chocolate bs" with returnSynonyms "true"
  Then a successful response is returned
  And the returned "synonyms" list is saved
  When the client searches text for pid "SITE;253" and query "bs chocolate" with returnSynonyms "true"
  Then a successful response is returned
...
Scenario: 11. Both terms where each term individually do not have synonyms (Compare with Scenario #8)
  When the client searches text for pid "SITE;253" and query "sugar blood" with returnSynonyms "true"
  Then a successful response is returned
  And the response contains array "synonyms"
    | synonym   |
    | blood     |
