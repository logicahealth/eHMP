...
#This feature item returns Demographics data in FHIR format from all VistA instances in which a patient record exists. It includes breaking the glass for a sensitive patient.
  ...
  Scenario: Client can request demographics in FHIR format
    ...
    When the client requests demographics for that patient "SITE;253"
  ...
  Scenario: Client can request demographics in FHIR format
    Given a patient with "demographics" in multiple VistAs
    When the client requests demographics for that patient "SITE;231"
    Then a successful response is returned
    And the results contain
      | demographics_supplemental_list     | demographics_values                                               |
  ...
  @F138_3_fhir_demographics  @US2344_SEN @fhir @SITE167
  Scenario: Client can break the glass when requesting demographics in FHIR format for a sensitive patient
    Given a patient with "demographics" in multiple VistAs
    When the client requests demographics for that sensitive patient "urn:va:patient:SITE:167:167"
    Then a permanent redirect response is returned
    #When the client breaks glass and repeats a request for demographics for that patient "urn:va:patient:SITE:167:167"
    #Then a successful response is returned
    And the results contain
     | demographics_supplemental_list                   | demographics_values                          |

  @F138_4_fhir_demographics @US2344_WIKI_test @fhir @SITE100022
  Scenario: Client can request demographics in FHIR format
    Given a patient with "demographics" in multiple VistAs
    When the client requests demographics for that patient "urn:va:patient:SITE:100022:100022"
    Then a successful response is returned
    And the results contain
      | demographics_supplemental_list | demographics_values |
      | resourceType                   | Patient             |
      ...
      | identifier.system | http://vistacore.us/fhir/id/uid   |
      | identifier.value  | urn:va:patient:SITE:100022:100022 |
       # identifier[y icn]
      | identifier.system | http://vistacore.us/fhir/id/lrdfn |
