 ...
 Scenario: Client can request Outpatient Medications in FHIR format
 	...
 	When the client requests out-patient medication results for the patient "SITE;100817" in FHIR format
 	Then a successful response is returned
 	And the FHIR results contain "medication dispense results"
 	| field                | panorama_value               |
 	| resource.resourceType | MedicationDispense           |
  | resource.status       | completed |
  | resource.dispenser.reference  | Provider/urn:va:user:SITE:20010 |
  | resource.contained.resourceType  | MedicationPrescription     |
  | resource.contained.patient.reference      | Patient/SITE;100817 |
  | resource.contained.identifier.system    | urn:oid:2.16.840.1.113883.6.233 |
  | resource.contained.identifier.value     | urn:va:med:SITE:100817:27831 |
  | resource.contained.status               | completed |
  | resource.contained.dateWritten          | IS_FHIR_FORMATTED_DATE |
  | resource.contained.prescriber.reference | Provider/urn:va:user:SITE:20010 |
  ...
  | resource.identifier.value | urn:va:med:SITE:100817:27831 |
...
And FHIR date and time conver to Zulu format for Outpatient Medication
 ...
 @F138_3_outpatient_medication_dispense_fhir @fhir @SITE167
 Scenario: Client can request out-patient medication results in FHIR format
 	Given a patient with "out-patient medication results" in multiple VistAs
 	#And a patient with pid "10104V248233" has been synced through the RDK API
 	When the client requests out-patient medication results for the patient "SITE;229" in FHIR format
 	Then a successful response is returned
 	And the FHIR results contain "medication dispense results"
 	| field                | panorama_value                        |
 	| resource.resourceType | MedicationDispense           |
  | resource.status       | completed |
  | resource.dispenser.reference  | Provider/urn:va:user:SITE:983 |
  | resource.contained.resourceType  | MedicationPrescription     |
  | resource.contained.patient.reference      | Patient/SITE;229 |
  | resource.contained.identifier.system    | urn:oid:2.16.840.1.113883.6.233 |
  | resource.contained.identifier.value     | urn:va:med:SITE:229:27952 |
  | resource.contained.status               | completed |
  | resource.contained.dateWritten          | IS_FHIR_FORMATTED_DATE |
  | resource.contained.prescriber.reference | Provider/urn:va:user:SITE:983 |
  ...
  | resource.identifier.system  | urn:oid:2.16.840.1.113883.6.233 |
  | resource.identifier.value | urn:va:med:SITE:229:27952 |
 ...
 Scenario: Client can break the glass when requesting out-patient medication results in FHIR format for a sensitive patient
       Given a patient with "out-patient medication results" in multiple VistAs
       When the client requests out-patient medication results for that sensitive patient "SITE;18"
 ...
 Scenario: Negative scenario.  Client can request Outpatient Medications in FHIR format
 	...
 	When the client requests out-patient medication results for the patient "SITE;100125" in FHIR format
 	Then a successful response is returned
 ...
 Scenario: Client can request Outpatient Medications statement in FHIR format
 	...
 	When the client requests out-patient medication statement for the patient "SITE;8" in FHIR format
 	Then a successful response is returned
 	And the FHIR results contain "medication dispense results"
 	 | field                        | panorama_value                      |
   | resource.resourceType 						| MedicationStatement |
   | resource.status                   | in-progress |
   | resource.patient.reference        | Patient/SITE;8 |
   | resource.contained.resourceType   | Medication |
   ...
   | resource.identifier.system                | urn:oid:2.16.840.1.113883.6.233 |
   | resource.identifier.value                 | urn:va:med:SITE:8:34267 |
   | resource.informationSource.reference      | Practitioner/urn:va:user:SITE:1 |
 ...
 Scenario: Client can request out-patient medication statement in FHIR format
 	...
 	When the client requests out-patient medication statement for the patient "SITE;149" in FHIR format
 	Then a successful response is returned
 	And the FHIR results contain "medication dispense results"
 	 | field                              | value                        |
   | resource.resourceType 						| MedicationStatement |
   | resource.status                   | in-progress |
   | resource.patient.reference        | Patient/SITE;149 |
   | resource.contained.resourceType   | Medication |
   ...
   | resource.identifier.system                | urn:oid:2.16.840.1.113883.6.233 |
   | resource.identifier.value                 | urn:va:med:SITE:149:18028 |
 ...
 Scenario: Client can break the glass when requesting out-patient medication statement in FHIR format for a sensitive patient
       Given a patient with "out-patient medication statement" in multiple VistAs
       When the client requests out-patient medication statement for that sensitive patient "SITE;18"
       Then a permanent redirect response is returned

 @F138_10_outpatient_medication_dispense_fhir @fhir @SITE167 @DE974
 Scenario: Client can request out-patient medication results in FHIR format
 	Given a patient with "out-patient medication results" in multiple VistAs
 	When the client requests out-patient medication results for the patient "SITE;229" in FHIR format
 	Then a successful response is returned
