...
Scenario: Client can request sync with immediate response in VPR RDK format
  Given the client requests that the patient with pid "9E7A;227" be cleared through the RDK API
  Then a successful response is returned
  When the client requests sync with immediate response within 30 second for the patient "9E7A;227" in RDK format
