...
Scenario: Create Endpoint for Calculate Days Supply (ORWDPS1 DFLTSPLY) (Medication)
  When the client requests picklist with the parameters for "medication-orders-default-days" with the user "9E7A;PW    "
      | paramter name | value                          |
      | patientDFN    | 0^                             |
      | unitStr       | 0                              |
      | schedStr      | 240                            |
...
Scenario: Create Endpoint for Calculate Quantity (ORWDPS2 DAY2QTY) (Medication)
  When the client requests picklist with the parameters for "medication-orders-quantity-for-days-supply" with the user "9E7A;PW    "
      | paramter name | value                                      |
      | patientDFN    | 146                                        |
      | daysSupply    | 30                                         |
      | unitsPerDose  | 1^                                         |
...
Scenario: Create Endpoint for Calculate Max Refills (ORWDPS2 MAXREF) Medication)
  When the client requests picklist with the parameters for "medication-orders-calc-max-refills" with the user "9E7A;PW    "
      | paramter name | value                              |
      | patientDFN    | 398                                |
      | days          | 90                                 |
      | ordItem       | 2086                               |
...
Scenario: Create Endpoint for Supplemental Drug Message (ORWDPS32 DRUGMSG) (Medication)
  When the client requests picklist with the parameters for "medication-orders-dispense-drug-message" with the user "9E7A;PW    "
      | paramter name | value                                   |
      | ien           | 5030                                    |
...
#Note:  pharmacyType = U/F/O is either U, F, or O - U = Unit Dose, F = IV Fluids, and O = Outpatient)
  When the client requests picklist with the parameters for "medication-defaults" with the user "9E7A;PW    "
      | paramter name | value                  |
      | pharmacyType  | O                      |
      | location   | 3 |
...
Scenario: Determines if the PKI Digital Signature is enabled on the site (ORWOR PKISITE)
  When the client requests picklist with the parameters for "pki-enabled" with the user "9E7A;PW    "
      | paramter name | value       |
...
Scenario: Retrieves Outpatient Medication ORDER QUICK VIEW file #101.44 subset of orderable items or quick orders in alphabetical order to specific sequence numbers (ORWUL FV4DG)
  When the client requests picklist with the parameters for "medication-list" with the user "9E7A;PW    "
      | paramter name | value           |
      | searchString  | O RX            |
...
Scenario: Get the index of a medication that meets a search term (ORWUL FVIDX)
  When the client requests picklist with the parameters for "medication-index" with the user "9E7A;PW    "
      | paramter name | value               |
      | ien           | 95               |
      | searchString  | ACES~ |
...
Scenario: Get the iOutpatient medication schedules (ORWDPS1 SCHALL)
  When the client requests picklist with the parameters for "medication-schedules" with the user "9E7A;PW    "
      | paramter name | value               |
...
Scenario: Get the Outpatient medication sOutpatient medication dosages, dispense, route, schedule, guideline, message, DEA schedule. (ORWDPS2 OISLCT)
  When the client requests picklist with the parameters for "medication-order-defaults" with the user "9E7A;PW    "
      | paramter name | value               |
      | ien           | 4937    |
      | pharmacyType  | O|
      | outpatientDFN | 204 |
...
Scenario: Get list of available Med Orders (ORWUL FVSUB)
  When the client requests picklist with the parameters for "medication-orders" with the user "9E7A;PW    "
      | paramter name | value             |
      | ien           | 95                |
      | first         | 1501              |
      | last          | 1600              |
