...
Scenario: Client can add an activity for my teams
  ...
  When the user "PW         " requests open activities for the patient context
    | parameter | value |
    | pid|SITE;3|
  Then a successful response is returned
...
#   | facility name    | PANORAMA (PAN) COLVILLE, WA |
  ...
  Then a successful response is returned
  Then the activity response does not contain the title
  And the activity response only contains activities started by USER  
  When the user "SITE;USER    " requests open activities for the patient context
    | parameter | value |
    | createdByMe|true|
    | pid|SITE;3|
  Then a successful response is returned
