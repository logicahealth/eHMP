...
Scenario: 
  ...
  When the "PW         " client requests the task is updated to be clarafication
      | parameter          | value            |
      | patient_pid        | SITE;100728      |
      | initiator          | 10000000016      |
      | route_facility     | 500              |
      | route_person       | SITE;10000000016 |
  ...
  And the task associated with the processInstanceId has
  | parameter  | value |
  | ASSIGNEDTO | SITE;10000000016,  |
  And the task result contains
  | parameter | value |
  | variables.name | state |
  | variables.value | Returned: Clarification Requested |
 ...
 Scenario: 
  ...
  When the "PW         " client requests the task is updated to be declined
      | parameter          | value            |
      | patient_pid        | SITE;100728      |
      | initiator          | 10000000016      |
      | route_facility     | 500              |
  ...
  When the "PW         " client requests tasks instances for "user" and "teamroles"
  Then a successful response is returned
  ...
  And the task result contains
    | parameter       | value |
    | variables.name  | state |
    | variables.value | Returned: Declined |
...
Scenario:
  ...
  And the user tdnurse has started an activity assigned to me with parameters
      ...
      | full_assignedTo  | SITE;10000000016 |
      | authorFac        | SITE             |
      | authorId         | 10000000016      |
      | authorName       | TDNURSE,ONE      |
  Then a successful response is returned
  ...
  When the "SITE;USER    " client requests the task is updated to be marked as complete
      | parameter      | value       |
      | patient_pid    | SITE;100728 |
      | initiator      | 10000000016 |
      | full_assignedTo| SITE;10000000016 |
...
Scenario:
  ...
  When the "SITE;USER    " client requests the task is updated for reassignment to 
      | parameter      | value            |
      | route_facility | 500              |
      | route_person   | SITE;10000000270 |
