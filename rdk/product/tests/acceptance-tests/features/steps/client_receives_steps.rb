...
class DefaultInitialize
  def site_initialize
    ...
    define_source_site['HDR'] = /urn:va:.*:ABCD/
...
class CountValue
  def count_source_allvalues(define_source_site, source_allvalues)
    ...
    @source_hdr = define_source_site["HDR"]
  ...
  def count_num_of_host(value)
    if !@source_panorama.match(value).nil?
      ...
      @num_dod_results += 1
    elsif !@source_hdr.match(value).nil?
      @num_hdr_results +=1
    else
      @not_define_value << value
    end
    num_results = { 'TOTAL' => @total_size, 'VISTA' => @num_panorama_results + @num_kodak_results, 'PANORAMA' => @num_panorama_results, 'KODAK' => @num_kodak_results, 'DOD' => @num_dod_results, 'HDR' => @num_hdr_results }
    return num_results
  end
  ...
  def find_site_value_from_source(define_source_site, count_source, site_name)
    ...
    unless count_source['TOTAL'] == count_source['PANORAMA'] + count_source['KODAK'] + count_source['DOD'] + count_source['HDR']
      puts '*************************************************************'
      puts '*                                                           *'
