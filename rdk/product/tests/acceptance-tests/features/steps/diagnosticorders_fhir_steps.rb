When(/^the client requests diagnosticorders for the patient "(.*?)" in FHIR format$/) do |pid|
  temp = QueryRDKFhir.new(pid, 'diagnosticorder')
...
Then(/^the FHIR results contain diagnosticorders/) do |table|
  # the dateformat for diagnosticorders appears to be different that what was defined as default
...
When(/^the client requests diagnosticorders for that sensitive patient "(.*?)"$/) do |pid|
