When(/^the client requests the ehmp-versions$/) do
  request = RDKQuery.new('ehmp-versions-list')
  @response = HTTPartyRDK.get(request.path)
end

And(/^the ehmp-versions has a key of "(.*?)"$/) do |key|
...
Then(/^the ehmp\-versions contains the attribute$/) do |table|
  response_body = JSON.parse(@response.body)
