...
class HTTPartyWithCookies
  ...
  @@time_out_time = 300
  # format  :xml
  def self.post_json_with_cookies(path, json, headers)
    @@cookie = HTTPartyWithBasicAuth.acquire_cookie if @@cookie.nil?
    p "POST JSON WITH COOKIE " + path
    options = {}
    options[:verify] = false
    options[:timeout] = @@time_out_time
    options[:body] = json
    options[:headers] = { 'Cookie' => @@cookie }.merge(headers)
  ...
  def self.delete_with_cookies(path)
    @@cookie = HTTPartyWithBasicAuth.acquire_cookie if @@cookie.nil?
    p "DELETE WITH COOKIE " + path
    options = {}
    options[:verify] = false
