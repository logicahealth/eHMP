...
class DefaultDateFormat
  include Singleton
  #Added regular expressions for the FHIR format of dates verification
  @@format = %r{ (\d{4}[-\/]\d{1,2}[-\/]\d{1,2}T\d\d:\d\d:\d\d[\+\-]\d\d:\d\d) |(\d{4}[-\/]\d{1,2}[-\/]\d{1,2}T\d\d:\d\d:\d\d[\+\-]\d\d:\d\dZ) | (\d{4}[-\/]\d{1,2}[-\/]\d{1,2}T\d\d:\d\d:\d\d\-\d\d:\d\dZ) | (\d{4}[-\/]\d{1,2}[-\/]\d{1,2}T\d\d:\d\d:\d\d\.\d{3}Z) | (\d{4}[-\/]\d{1,2}[-\/]\d{1,2}T\d\d:\d\d:\d\d\.\d{3}\-\d\d:\d\d) | (\d{4}[-\/]\d{1,2}[-\/]\d{1,2}T\d\d:\d\d:\d\d\.\d{3}[\+\-]\d\d:\d\d)}
...
class JsonFieldDateValidator
  ...
  #Added method to handle FHIR date format
  def all_matches_fhir_date_format(fieldpath, fhirdateformat, jsonobject)
