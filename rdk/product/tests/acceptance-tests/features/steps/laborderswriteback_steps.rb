...
class LabOrdersByTypeQuery < BuildQuery
  def initialize(parameter_hash_table)
    ...
    # /patient/9E7A;100470/orders?accessCode=REDACTED&verifyCode=REDACTED&site=9E7A&pid=9E7A;100470"
    domain_path = RDClass.resourcedirectory.get_url(title)
    p domain_path
    p "domain path: #{domain_path}"
    #@path.concat(domain_path) 
    @path = "#{writeback_url}/resource/resourcedirectory/write-health-data/patient/9E7A;100470/orders?accessCode=REDACTED&verifyCode=REDACTED&site=9E7A&pid=9E7A;100470" 
    p "path: #{path}"
    p @path
...
When(/^the client request a response in VPR format from RDK API with the parameters$/) do
  ...
  HTTParty.post("http://IP             /resource/write-health-data/patient/9E7A;100472/orders?accessCode=REDACTED&verifyCode=REDACTED&site=9E7A&pid=9E7A;100472",
                {
...
When(/^the client updates order in VPR format from RDK API with the parameters$/) do 
  ...
  HTTParty.put("http://IP             /resource/write-health-data/patient/9E7A;100472/orders/order_id;1?accessCode=REDACTED&verifyCode=REDACTED&site=9E7A&pid=9E7A;100472",
               { 
...
When(/^the client discontinues order in VPR format from RDK API with the parameters$/) do 
  ...
  HTTParty.delete("http://IP             /resource/write-health-data/patient/9E7A;100472/orders/order_id;1?accessCode=REDACTED&verifyCode=REDACTED&site=9E7A&pid=9E7A;100472",
                  { 
