When(/^the client requests the permission set features list$/) do
  request = RDKQuery.new('permission-sets-features')
  @response = HTTPartyRDK.get_as_user(request.path, "SITE;USER  ", "PW      ")
end

Then(/^the permission set features list contain the attributes$/) do |table|
