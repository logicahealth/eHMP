...
class JobDestroyer {
    ...
    def run(){
		...
		jenkinsMain.job.grep{ job -> job.name.text().contains('solr-smart-client') && job.name.text() != 'creator' && job.name.text() != 'destroyer' && !job.name.text().contains('template')}.each{ job ->
