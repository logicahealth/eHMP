...
var parseMessage = require('../../src/RpcSender').RpcSender.parseMessage;
...
function testCommandMultiRpc(command, sentErrors, sentResults, expectedError, expectedResult, overrideConfig) {
    var testError;
    var testResult;
    var called = false;
    ...
    var instance = {
        ...
        sender: {
            send: function send(string, callback) {
                callback(sentErrors.shift(), sentResults.shift());
            },
            close: _.noop
        }
    };
    ...
    waitsFor(function() {
        return called && sentErrors.length === 0 && sentResults.length === 0;
    }, 'should be called', 1000);
...
describe('RpcClient.js', function() {
    ...
    describe('divisionCommand()', function() {
        it('verify doesnt call divisionCommand when no division present', function() {
            testCommand(RpcClient.prototype.divisionCommand, null, null, null, 0);
        });

        it('verify it returns the error when DIVISION GET fails', function() {
            var testConfig = _.clone(config);
            testConfig.division = '500';
            testCommandMultiRpc(RpcClient.prototype.divisionCommand, ['explode'], [null], 'explode', null, testConfig);
        });

        it('verify it returns the error when GET USER INFO fails', function() {
            var testConfig = _.clone(config);
            testConfig.division = '500';
            testCommandMultiRpc(RpcClient.prototype.divisionCommand, [null, 'explode'], ['0\r\n', null], 'explode', null, testConfig);
        });

        it('verify returns the correct division when user is assigned to the division in single division site', function() {
            var testConfig = _.clone(config);
            testConfig.division = '500';
            testCommandMultiRpc(RpcClient.prototype.divisionCommand, [null, null], ['0\r\n', '10000000270\r\nUSER,PANORAMA\r\nPanorama User\r\n500^CAMP MASTER^500\r\n\r\nMEDICINE\r\n\r\n4000\r\n\r\n'], null, '500', testConfig);
        });

        it('verify returns the correct division when given valid non-string division', function() {
            var testConfig = _.clone(config);
            testConfig.division = 500;
            testCommandMultiRpc(RpcClient.prototype.divisionCommand, [null, null], ['0\r\n', '10000000270\r\nUSER,PANORAMA\r\nPanorama User\r\n500^CAMP MASTER^500\r\n\r\nMEDICINE\r\n\r\n4000\r\n\r\n'], null, '500', testConfig);
        });

        it('verify returns the correct division when user is assigned to the division in multidivision site', function() {
            var testConfig = _.clone(config);
